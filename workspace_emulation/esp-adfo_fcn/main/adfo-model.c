...
#include "MNIST_train_data.h"
#include "MNIST_train_data_label.h"
#include "MNIST_val_data.h"
#include "MNIST_val_data_label.h"
...
#define MNIST_NEURONS_1     12
#define MNIST_OUTPUTS       10
#define MNIST_LAYER_CNT     4

aiopti_adam_f32_t adam_opti = AIOPTI_ADAM_F32(0.01f, 0.9f, 0.999f, 1e-8f);

uint16_t input_layer_shape[] = {1, MNIST_INPUTS};
ailoss_crossentropy_f32_t model_loss;

ailayer_input_f32_t		input_layer	        = AILAYER_INPUT_F32_A(2, input_layer_shape);
ailayer_dense_f32_t		dense_layer_1	    = AILAYER_DENSE_F32_A(MNIST_NEURONS_1);
ailayer_relu_f32_t		relu_layer_1 	    = AILAYER_RELU_F32_A();
ailayer_dense_f32_t		dense_layer_2	    = AILAYER_DENSE_F32_A(MNIST_OUTPUTS);
...
void show_labels() {
    int training_size = sizeof(MNIST_training_data) / sizeof(MNIST_training_data[0]) / (image_size*image_size);
    int labels[10] = {0};

    for (int i = 0; i < training_size*10; i++) {
        if (MNIST_training_data_label[i] == 1.0f) {
            labels[i%10]++;
        }
    }
...
void eval_test_data(float *output_data, int test_size){
    int r = (rand() % (test_size - 5)/10)*10; // Pick a random number up to test size as multipliers of 10
    DEBUG("\n----- Test data starting from %d, Predicted Values | Target Values -----\n", r);
    for (int i=r; i < r+10; i++){
        DEBUG("\t%f\t%f |", output_data[i],MNIST_test_data_label[i]);
        DEBUG("\t%f\t%f |", output_data[i+10],MNIST_test_data_label[i+10]);
        DEBUG("\t%f\t%f |", output_data[i+20],MNIST_test_data_label[i+20]);
        DEBUG("\t%f\t%f |", output_data[i+30],MNIST_test_data_label[i+30]);
        DEBUG("\t%f\t%f |", output_data[i+40],MNIST_test_data_label[i+40]);
        DEBUG("\n");
    }
}
...
void calc_success_rate(float *output_data, int test_size){
    ...
    float success=0;
    for (int i=0; i < test_size*10; i+=10){
        predicted_index=i;
        target_index=i;
        ...
        for (int j=0; j<10;j++){
            if (MNIST_val_data_label[i+j] >= MNIST_val_data_label[target_index]){
                    target_index=i+j;
            }
        }
...
void calc_success_rate_val(float *output_data, int test_size){
    ...
    for (int i=0; i < test_size*10; i+=10){
        ...
        for (int j=0; j<10;j++){
            if (MNIST_test_data_label[i+j] >= MNIST_test_data_label[target_index]){
                    target_index=i+j;
            }
        }
        if (target_index == predicted_index) success++;
...
void calc_macro_f1_score() {
    int test_size = 100;
    static int TP[10] = {0};
    static int FP[10] = {0};
    static int FN[10] = {0};

    // Step 1: Count TP, FP, FN for each class
    for (int i = 0; i < test_size; i++) {
        int true_label = true_labels_array[i];
        int pred_label = pred_labels_array[i];
    ...
    LOG("Label operations done!\n");
    float f1_sum = 0.0f;
    for (int c = 0; c < 10; c++) {
        float precision = 0.0f, recall = 0.0f, f1 = 0.0f;

        if(TP[c] + FP[c] > 0){
            precision = (float)TP[c] / (TP[c] + FP[c]);
            LOG("Precision %f\n", precision);
        }
        if(TP[c] + FN[c] > 0){
            recall = (float)TP[c] / (TP[c] + FN[c]);
            LOG("recall %f\n", recall);
        }
    ...
    float macro_f1 = f1_sum / 10;
    LOG("#### Macro F1 Score: %f\n", macro_f1);
    return;
}
...
void init_model(aimodel_t *local_model, aiopti_t *model_optimizer, void* parameter_memory, void* working_memory)
{
    ...
    x = ailayer_dense_f32_default(&dense_layer_1, local_model->input_layer);
...
void train_model(aimodel_t *local_model, aiopti_t *model_optimizer){
    ...
    float *subset_training_data_label = (float*)malloc(subset_size * sizeof(float) * 10);
    int selected_index[subset_size] = {};
    memset(selected_index, 255, sizeof(selected_index));
    ...
    for (int i=0; i<subset_size; i++){
        ...
        for(int j = 0; j < (image_size  * image_size); j++){
            subset_training_data[i * (image_size  * image_size) + j] = MNIST_training_data[starting_point + j];
        }
        int starting_point_label = index * 10;
        for(int j = 0; j < 10; j++){
            subset_training_data_label[i * 10 + j] = MNIST_training_data_label[starting_point_label + j];
        }
    }
    ...
    // int training_size = sizeof(MNIST_training_data) / sizeof(MNIST_training_data[0]) / (image_size*image_size);
    int test_size = sizeof(MNIST_val_data) / sizeof(MNIST_val_data[0]) / (image_size*image_size);
    ...
    uint32_t i;
    uint16_t input_shape[] = {training_size, image_size * image_size}; // [sample count, channels, height, width]
    aitensor_t input_tensor = AITENSOR_2D_F32(input_shape, subset_training_data);
    uint16_t target_shape[] = {training_size, 10};
	aitensor_t target_tensor = AITENSOR_2D_F32(target_shape, subset_training_data_label);
	

    uint16_t test_shape[] = {test_size, image_size * image_size}; // [sample count, channels, height, width]
	aitensor_t test_tensor = AITENSOR_2D_F32(test_shape, MNIST_val_data);
...
void load_params(aitensor_t *trainable_parameters, aimodel_t *local_model){
    ...
    int test_size = sizeof(MNIST_test_data) / sizeof(MNIST_test_data[0]) / (image_size*image_size);

    uint16_t test_shape[] = {test_size, image_size * image_size}; // [sample count, channels, height, width]
	aitensor_t test_tensor = AITENSOR_2D_F32(test_shape, MNIST_test_data);
