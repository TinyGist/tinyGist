...
{   
    ...
    total_receives = (float *) malloc(RECEIVE_ARRAY_SIZE * sizeof(float));
    if (total_receives == NULL) {
        printf("Failed to allocate total_receives in PSRAM\n");
        abort();
    }
    memset(total_receives, 0, RECEIVE_ARRAY_SIZE * sizeof(float));
    received_flatten = (float*) malloc(RECEIVE_ARRAY_SIZE * sizeof(float));
    if (received_flatten == NULL) {
        printf("Failed to allocate received_flatten in PSRAM\n");
        abort();
    }
    memset(received_flatten, 0, RECEIVE_ARRAY_SIZE * sizeof(float));
